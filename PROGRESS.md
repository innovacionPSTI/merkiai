...
- [~] **v65 — HU-158 · Etapa 3 (cierre: config + query-functions) · 🟡:** las secciones que usaban **query-functions con defaults** (service-role + tenant default) seguían mostrando el default. Cableadas al tenant: páginas `configuracion/{general,emails,legal,envios,pagos,temas}`, `cupones`, `variantes`, `sistema/apariencia`; rutas `config`, `shipping`, `payment-config`, `pickups`, `coupons(+[id])`, `variant-types(+[id])` → `getAdminUser()+getAdminDb()`; `createCoupon`/`createVariantType` aceptan `tenantId` y lo setean en el insert. **Verificado por grep:** ya **no queda** ningún `createServerClient()` ni query-function con default en el admin, salvo lo legítimo: `auth.ts` (lookup de `profiles` para resolver el tenant), `owners` (control-plane), `upload` (Storage). **Único write pendiente:** `import` (restore masivo — su `onConflict` debe volverse por-tenant; herramienta de backup poco usada, se deja documentada). admin **194** · database **171** · type-check web/admin limpios. **El admin quedó tenant-scoped de punta a punta.**
- [~] **v64 — HU-158 · Admin tenant-scoped · Etapa 3 (rewiring completo) · 🟡→casi:** migración **`e17/10_rls_admin_all.sql`** (DO idempotente): políticas `*_admin_all` para orders/customers/customer_addresses/profiles/pages/page_sections/section_items/blog_posts/nav_items/coupons/testimonials/themes/media_assets/banners/newsletter_subscribers/shipping_profiles + los 4 config. **Cableado a `getAdminDb` (RLS por tenant) de TODO el panel:** helper `requireAdminDb()` (páginas) + patrón `getAdminUser()+getAdminDb()` (rutas). **13 páginas** (dashboard/pedidos/clientes/usuarios/categorias/contenido/blog/nav/media/newsletter/productos) + **rutas** categorias, usuarios, themes, media, newsletter, export, pedidos (status/notes/payment-status/reconcile), blog, nav, cms, upload → todas acotadas por tenant; los inserts setean `tenant_id`. `types.ts`: `tenant_id` añadido a ~14 tablas que faltaban. **Único service-role restante (legítimo):** `auth.ts` (lookup de `profiles` para *resolver* el tenant), `owners` (control-plane), `upload` (Storage), e **`import`** (restore masivo — **pendiente**: `onConflict` por-tenant en singletons). admin **194** · database **171** · type-check web/admin limpios. **Requisito despliegue:** aplicar `e17/09` (con `variant_types`) + **`e17/10`** + `SUPABASE_JWT_SECRET` en el admin. **Con esto, el admin de un tenant solo ve/edita SUS datos** (dashboard/pedidos/clientes/productos/contenido/config).
- [~] **v63 — HU-158 · Admin tenant-scoped · Etapa 2 (cliente RLS + piloto productos) · 🟡:** infra del cliente RLS del admin: `mintTenantJwt` acepta `claims` (p. ej. `is_admin`); **`getAdminDb(tenantId)`** (`apps/admin/src/lib/admin-db.ts`) = cliente `authenticated` + `tenant_id` + `is_admin` (fail-closed, exige `SUPABASE_JWT_SECRET`). Migración **`e17/09_rls_admin.sql`** (idempotente, aditiva): políticas **`*_admin_all`** (`FOR ALL TO authenticated USING/WITH CHECK tenant_id=jwt.tenant_id AND is_admin`) para el **módulo productos** (products/product_variants/categories). **Módulo productos cableado completo:** `POST /api/admin/products` + `[id]` GET/PATCH/DELETE + páginas `productos/page.tsx` (listado) y `productos/[id]/page.tsx` (edición) usan `getAdminUser().tenantId` + `getAdminDb` (ya no service-role; el listado antes leía **sin filtro → productos de TODOS los tenants**). Insert/upsert setean `tenant_id`; PATCH ignora `tenant_id` del cliente. `getVariantTypes` queda en service-role hasta que Etapa 3 le agregue política. `@merkiai/tenancy` agregado a `apps/admin` (package.json + lockfile). Tests actualizados (products-limit, products-create); admin **194** · type-check web/admin limpio. **Requisitos de despliegue:** aplicar `e17/09` + `SUPABASE_JWT_SECRET`/`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` en el entorno del **admin**. **Etapa 3:** replicar `*_admin_all` + cableo a las ~22 tablas/módulos restantes (dashboard/pedidos/usuarios/config/contenido) → hasta entonces esos siguen en service-role (default).
- [~] **v62 — HU-158 · Admin tenant-scoped · Etapa 1 (resolución de tenant por profiles) · 🟡:** `getAdminUser()` ahora resuelve **el tenant que administra el usuario desde `profiles.tenant_id`** (decisión aprobada) y lo devuelve (`AdminUser.tenantId`): busca TODOS los perfiles del email, elige por la **cookie de tenant activo** o el único perfil admin; si el correo tiene perfil admin en >1 tienda y ninguna activa → `needsWorkspaceSelection` y el layout redirige a `/seleccionar-tienda` (sin loop). **Ya no traga el error** de la consulta (antes: email con perfiles en varios tenants rompía `maybeSingle` en silencio → "Sin acceso"). Test `auth.test.ts` (6). Admin **194** · type-check limpio. **Pendiente HU-158 (Etapas 2–4):** cliente **RLS por tenant** para el admin (deja service-role) + políticas de escritura por tenant + **rewiring** de todas las queries/rutas al tenant activo (dashboard/usuarios/catálogo/pedidos/config) → hasta entonces **el admin sigue mostrando datos del default** (la Etapa 1 solo fija la identidad/tenant, no cierra la fuga de datos).
- [~] **v61 — HU-216 · Config-por-tenant en webhooks/reconcile/checkout · 🟡:** cierre de la deuda allowlisted de v60. Los **5 webhooks** de pago/envío resuelven el tenant **por su host** (`resolveTenant()`; el checkout ya configura las URLs de callback con el subdominio del tenant) → `getPaymentConfig/getStoreConfig(supabase, tenantId)` en vez del default (antes: llaves de pasarela del default para tenants no-default). **Skydropx** usa `order.tenant_id`. **Reconcile** (bold/tucompra) y **shipping/shipments** leen la config del `order.tenant_id`. **Checkout:** `getPaymentConfig` por tenant (resolución movida antes de leer la config) + `saveAddressForUser` setea `tenant_id`. Tests: mock de `resolveTenant` en webhook-bold/wompi/mercadopago. Web **344** verde · type-check + lint (0 violaciones) limpios. **Requisito operativo:** cada tenant configura la URL de webhook con su subdominio. **Pendiente HU-216:** auditar `blog`/`[slug]`/PDP (catálogo y home ya scoped).
- [~] **v60 — HU-216 · Auditoría anti service-role en el storefront + enforcement por lint · 🟡:** `createServerClient` (service-role) **omite RLS** → en el plano de tienda es fuga cross-tenant. Auditado todo `apps/web`. **Fugas corregidas:** `(account)/account/profile` y `checkout/page` leían `customers` con service-role → `getRequestUserDb` (RLS `authenticated`); `api/auth/welcome` (registro del comprador) y `api/newsletter` no ponían `tenant_id` → resuelven tenant por host + `ensureCustomer(tenantId)` / upsert `tenant_id` (**causa de "el comprador caía al default"**); `account/page` quita el import de service-role. `types.ts`: `newsletter_subscribers.tenant_id`. **Enforcement:** `apps/web/.eslintrc.json` con `no-restricted-imports` que **prohíbe `createServerClient` en el storefront** (error de lint), allowlist solo para máquina-a-máquina (webhooks/reconcile/checkout/shipping) — verificado que dispara. Web **344** · database **171** · type-check + lint limpios. **Pendiente (HU-216, allowlisted):** webhooks/reconcile leen `payment_config`/`store_config` **sin tenant** (config del default → llaves de pasarela equivocadas para tenants no-default): resolver `order.tenant_id`; `checkout saveAddress` con `tenant_id`.
- [~] **v59 — HU-207/HU-157 · Storefront tenant-scoped (tema/nav/config/home) + redirect de host sin tenant · 🟡:** `(public)/layout` + root layout + `generateMetadata` leen `store_config`/nav/footer/**tema**/título/favicon del **tenant resuelto por host** (cliente RLS anon + `tenantId`), no del default. **Fix home:** `getWebHomeData(db)` pasaba el cliente tenant-scoped solo a `page_sections`; `getFeaturedProducts`/`getBlogPosts`/`getBestSellingProducts`/`getCategories` corrían **sin `db`** → default; ahora reciben `db`. Migración **`e17/08_rls_themes.sql`** (RLS anon-tenant de `themes`; sin ella el storefront no lee tema por RLS). **Redirect** en el middleware: host sin tenant (404 definitivo) → `https://merkiai.com` (apex excluido, fail-safe ante error/duda). **✅ VALIDADO en vivo con `prueba1`:** catálogo/tema/nav/marca por tenant; tienda vacía sin la marca del default. Web **344** · database **171** verde. **Pendiente:** auditar blog/`[slug]`/PDP (parte de HU-216).
- [~] **v58 — HU-215 · Aprovisionamiento idempotente del Team (Stack Auth) · ✅ (a/b/c):** Stack Auth permite display names repetidos → `createTeam` duplicaba Teams en reintentos. `ensureTenantTeam(tenantId,name,plan)`: reusa `stack_team_id` si existe **y sigue vivo** (`orgExists`), si falta o quedó **referencia muerta** (Team borrado a mano) recrea y re-persiste. `provisionTenant` y "Invitar dueño" lo usan → idempotente y auto-reparable. `IdentityProvider.deleteOrg`/`orgExists` + adaptador Stack. Console **34** verde.
- [~] **v57 — HU-209 · Acceso del dueño = super_admin + seguridad del endpoint interno · 🟡:** El admin autoriza por `profiles.role` (BD del admin), **no** por el Team. **Causa raíz de "Sin acceso" (sin logs):** el middleware del admin exigía sesión en TODA ruta → el `POST` server-to-server de la consola a `/api/internal/owners` era redirigido a `/login` (307) y el `fetch` lo tomaba como éxito → el `profiles` nunca se creaba. **Fixes:** (1) middleware del admin deja pasar `/api/internal`; (2) endpoint **`POST /api/internal/owners`** (control plane) crea el `profiles {super_admin, tenant_id}` del dueño — **único** origen válido de super_admin (el admin lo tiene prohibido); (3) **guard centralizado `withInternalAuth`** (fail-closed 500 si secreto <24, rate-limit por IP 429, secreto timing-safe 401) + **logging** de fallos → ninguna ruta `/api/internal` puede olvidar el chequeo; (4) helper de la consola con `redirect:'manual'` (un 3xx ya no es falso éxito); (5) fix del loop de `/no-autorizado` ("Iniciar sesión con otra cuenta" → sign-out). `profiles.tenant_id` tipado. Admin **188** · console **34** verde. **✅ VALIDADO en vivo:** dueño creado como super_admin. **Decisión aprobada:** dueño = super_admin; cupos de admin por plan (HU-173 AC-7). **Pendiente (config despliegue):** allowlist de red consola↔admin, secreto de aprovisionamiento dedicado.
- [~] **v56 — HU-209 · Aprovisionamiento operable desde la consola · 🟡:** surface de errores en el form (`useActionState`: error/éxito/warnings; antes el fallo de `createOrg` era invisible) + **selector de plan** (default `free`); `provisionTenant` **no borra el tenant** si falla el Team → modo **parcial** con warning (coherente con la UI); `callbackUrl` en `inviteMember` (Stack Auth lo exige en servidor); **gestión por tenant** (invitar/cambiar dueño + eliminar con "Zona de peligro" type-to-confirm; bloquea el tenant por defecto). Console tests + type-check limpios. **Este bloque (v56–v60) es la iteración de HU-209/215/207/216 validada en vivo con el tenant `prueba1`.**
- [~] **v55 — HU-158 · Switcher de tienda + tema por tenant activo (Step 2) · 🟡:** en el topbar del admin, **switcher de tienda** (nombre del workspace activo + enlace a `/seleccionar-tienda`) que aparece **solo si el operador tiene >1 tienda** (`listWorkspaces()` guarded). El `admin_config` (tema del panel) del layout ahora se lee por **tenant activo** (`getAdminConfig(undefined, activeTenantId)`). Admin **175** tests + type-check limpio. **Pendiente HU-158:** scoping del resto de queries del admin (productos/pedidos/contenido + las ~16 lecturas de config en pages/rutas) al tenant activo, y forzar el resolutor post-login cuando haya tenants provisionados.
- [~] **v54 — HU-214 · Guardias de sesión en los 3 logins · 🟡:** `useUser()` + `router.replace` en los logins de **web** (`LoginForm`), **console** y **admin** (esta última en v53) → si ya hay sesión (tras magic link/OAuth) reenvía; arregla el no-redirect de Google. Type-check limpio (web/console/admin). **Pendiente HU-214 (config en Stack Auth, no código):** OAuth propio por proyecto (adiós Shared keys), account-linking por email verificado, Trusted Domains base (web wildcard + dominios propios; admin/console host fijo), revisión de cookieDomain host-scoped/`afterSignIn`.
- [~] **v53 — HU-158 · Selector de workspace + tenant activo (Step 1) · 🟡:** `active-tenant.ts` (cookie httpOnly `merkiai_active_tenant` + get/set, fallback default), `workspaces.ts` (`listWorkspaces` vía Stack Auth `user.listTeams()` + `serverMetadata.tenant_id` [de HU-209], guarded; `resolveWorkspace` puro: none/auto/active/select), página **`/seleccionar-tienda`** (estilo VoxAlly adaptado: tarjetas por tienda + logout) + acción `selectWorkspace` (verifica pertenencia → set cookie → /dashboard). `entitlements.ts` usa el **tenant activo** (cookie) en vez de `ADMIN_TENANT_ID`. **Guardia de sesión** en el login del admin (`useUser` → redirige si ya hay sesión) → arregla el no-redirect de Google. Pruebas: `workspaces.test.ts` (5). Admin **175** · type-check limpio. **Pendiente HU-158:** forzar el resolutor post-login (cuando haya tenants provisionados con Team+metadata; hoy no, para no bloquear al operador actual del tenant default), switcher en topbar, y scoping de todas las queries del admin al tenant activo.
- [~] **v52 — HU-207 · Config por tenant (`store_config`) · 🟡:** migración idempotente `e17/07_config_por_tenant.sql`: **UNIQUE(tenant_id)** en `store_config`/`payment_config`/`admin_config`/`shipping_config` (una config por tenant) + **RLS**: `store_config` lectura anon acotada por tenant (info pública) y `payment_config`/`admin_config`/`shipping_config` **RLS habilitado sin política** (secretos/interno → solo service-role). `getStoreConfig(db, tenantId=DEFAULT)`/`updateStoreConfig(...)` pasan de singleton (`limit(1).single()`/`upsert id=1`) a **por-tenant** (`.eq('tenant_id')`/`upsert onConflict:'tenant_id'`), preservando el tenant por defecto para los callers actuales. `types.ts`: `tenant_id` en `store_config`. Pruebas: `store-config.test.ts` reescrito (13, incl. filtro por tenant + upsert por tenant). Database **171** · admin **170** · type-check limpio (web/admin). **Actualización:** el mismo patrón se aplicó a **payment_config, admin_config y shipping_config** (get por `tenant_id`, update→**upsert onConflict:'tenant_id'`**; `tenant_id` tipado; `admin-config.test` reescrito). Los 4 singletons ya son por-tenant. Database **171** verde. **Pendiente HU-207:** blog+nav host-scoping y **cablear callers** (layout/checkout/admin) para pasar el tenant resuelto (hoy default; el de admin depende de HU-158).
- [x] **v51 — Consola: Email OTP (magic link) en vez de Google · ✅:** `LoginScreen` (`@merkiai/ui`) gana la prop opcional **`onMagicLink`** (botón "Enviar enlace de acceso al correo" + confirmación); Google (`onGoogle`) intacto para web/admin. La **consola** deja email+contraseña + **magic link** (`app.sendMagicLinkEmail`), sin SSO (decisión del operador). Type-check limpio (console/web/admin) · console 23 tests.
- [~] **v50 — HU-209 · Aprovisionamiento de tenant (núcleo) · 🟡:** `lib/provisioning.ts` → `provisionTenant(input, {db, adminIdentity})`: valida+reserva subdominio (único), inserta `tenants`, **crea el Team en Stack Auth (proyecto admin)** con `serverMetadata {tenant_id,plan,status}` + guarda `stack_team_id` + **invita al dueño** por email; **rollback** (borra el tenant) si falla el Team. `lib/admin-identity.ts` construye el `IdentityProvider` del proyecto admin desde env (`NEXT_PUBLIC_ADMIN_HEXCLAVE_*` + `ADMIN_HEXCLAVE_SECRET_SERVER_KEY`); si faltan → provisioning **parcial** (tenant+subdominio, Team luego). `createTenant` cableado + campo "email del dueño" en la UI. Pruebas: `provisioning.test.ts` (5: éxito/parcial/subdominio en uso/inválido/rollback). Console **23** tests + type-check limpio. README con las envs del proyecto admin. **Pendiente HU-209:** seed de config por-tenant (HU-207), rol por defecto del dueño (Team Permission), y verificación en vivo (crear un tenant real end-to-end).
- [x] **v49 — Login con Google (Gmail) en tienda y admin · ✅:** habilitado en Stack Auth (los 3 proyectos). Cableado: **admin** (`login/page.tsx` → `onGoogle` de `LoginScreen` → `signInWithOAuth('google')`) y **web** (`LoginForm.tsx`: botón "Continuar con Google" con ícono + divisor + handler). La consola ya lo tenía. Type-check limpio (web/admin).
- [x] **v48 — HU-210 · Design system de paneles + HU-211 · Rediseño de la consola · ✅:** biblioteca presentacional en `@merkiai/ui` (`panel.tsx`, estilos inline + variables CSS, temable, sin Tailwind): `PanelShell` (grid sidebar+topbar responsive), `PanelSidebar` (grupos + íconos + activo + footer), `PageHeader`, `PanelCard`, `StatCard`/`StatGrid`, `StatusBadge` (tones success/danger/warn/info/neutral), `EmptyState`. **Consola** rediseñada con ello: sidebar agrupado "Plataforma" (Tenants/Planes con íconos, Dominios/Auditoría próximamente), topbar (Control Plane + operador + logout), **StatCards** en Tenants (total/activos/suspendidos/planes), tablas en `PanelCard` con `StatusBadge` de estado y `EmptyState`. Console **18** tests + type-check limpio (console/web/admin). Abstracción de patrones SaaS, sin dominio de terceros. **Pendiente:** HU-212 (aplicar al admin).
- [x] **v47 — HU-208 · Shell de la consola (nav + logout + sesión) · ✅:** `apps/console` deja de ser una página pelada: **topbar** sticky (marca + email del operador + `LogoutButton`), **sidebar** de navegación (Tenants · Planes · Dominios/Auditoría "próximamente") con anclas a las secciones, grid responsive (colapsa el sidebar <760px). `LogoutButton` (client) usa `useStackApp().signOut()` → redirige a `/login` (afterSignOut). **Corrección:** el nav ahora navega entre **páginas separadas** (grupo `(dash)`: `/` = Tenants, `/planes` = Planes) con resaltado activo (`Sidebar` client + `usePathname`), en vez de anclas a una sola página; tablas anchas con scroll horizontal + grid `minmax(0,1fr)` (arregla el desborde/espacio en blanco). Console **18** tests + type-check limpio.
- [~] **v46 — HU-173 · Gestión de planes en la consola + límite de productos · 🟡:** Consola: **CRUD de planes** (`plan-validation.ts` puro + `savePlan` upsert por key con features/limits JSON validados; `getPlans`) y **asignación de plan a tenant** (`setTenantPlan`, gated `platform:operate`); UI con tabla de planes, form crear/editar y selector de plan por tenant. Endpoint interno `GET /api/internal/tenants/[id]` ahora devuelve **entitlements** (features/limits del plan). Admin: `lib/entitlements.ts` (consulta el control plane; `withinLimit`/`PlanEntitlements` inline para no acoplar) + **enforcement del límite de productos** en `POST /api/admin/products` (403 al superar; fail-open si el control plane no está configurado, por ser regla de negocio). `types.ts`: `tenant_id` añadido a `products`. Pruebas: `plan-validation.test.ts` (8) + `products-limit.test.ts` (403). Console **18** · admin **170** · database **173** verdes, type-check limpio (web/admin/console). **Pendiente HU-173:** límite de **usuarios** (cuenta de team en Stack Auth → depende de HU-158) y límites en el onboarding. `ADMIN_TENANT_ID` es interino hasta HU-158 (tenant del team del operador).
- [x] **v45 — Hardening RLS: fail-closed sin flag ni service-role · ✅:** eliminada toda vía fail-open del plano de tienda. `getRequestUserDb`/`getRequestCatalogDb` **exigen `SUPABASE_JWT_SECRET`** y **lanzan** si falta (antes: degradaban a service-role). Quitado el flag `SESSION_RLS_ENABLED` y los `?? createServerClient()` de perfil/direcciones/`addresses/[id]`/carrito; `getDefaultAddress` exige el cliente. El comprador **solo** lee vía `authenticated`; escrituras privilegiadas (crear pedido/stock/webhooks/`ensureCustomer`) siguen server-to-server. Tests: `tenant-db.test.ts` (lanza sin secreto), `profile`/`addresses-id` migrados. Web **35·344** verde, type-check limpio. **Requisito de despliegue:** `SUPABASE_JWT_SECRET` en TODO entorno de `web` + `e17/02/05/06` aplicadas, o el storefront falla explícito (por diseño). **Faltante registrado como HU-207** (blog/nav por host + config singletons por tenant).
- [~] **v44 — HU-156 · RLS de contenido por tenant (`e17/06`) · 🟡:** migración idempotente `e17/06_rls_content.sql`: reemplaza las políticas `*_public_read` **amplias** (fuga cross-tenant) por `*_tenant_read` acotadas por `tenant_id` en `pages`/`page_sections`/`section_items`/`blog_posts`/`nav_items` (rol anon+authenticated; service-role sigue omitiendo RLS). Página CMS `(public)/[slug]` cableada a `getRequestCatalogDb()` (host→tenant). Web **35·344** verde, type-check limpio. **Aplicar `e17/06` en staging** (fail-closed sin JWT de tenant, igual que catálogo). **Pendiente HU-156/158:** blog + Navbar (`nav_items`) al cliente por host; **config singletons por tenant** (`store_config`/`admin_config`/`payment_config`/`shipping_config` → fila-por-tenant + seed en onboarding). Con eso + `SESSION_RLS_ENABLED` validado, HU-156 queda cerrado.
- [~] **v43 — HU-156 · Carrito + `addresses/[id]` cableados + fix best-sellers · 🟡:** `api/account/cart` (GET/POST/DELETE) y `api/account/addresses/[id]` (PATCH/DELETE) pasados a `ensureCustomer` + cliente `authenticated` (RLS `cart_own`/`addresses_own`) + `tenant_id` en escrituras (`cart_items` tipado con tenant_id). **Fix best-sellers (deuda C1):** `getBestSellingProducts` ya NO consulta la tabla inexistente `order_items`; agrega ventas desde `orders.items` (JSONB) por variante→producto (excluye `cancelled`), con fallback a recientes. Ningún código referencia ya `order_items` → habilita una regeneración limpia de `types.ts`. Pruebas: `products.test.ts` +2 (agregación + fallback); `addresses-id` migrado (6). Web **35·344** · database **18·173** verdes, type-check limpio. **Pendiente HU-156:** singletons no-catálogo (`store_config`/`home_sections`/`pages`/`blog`) con `tenant_id`+RLS, y **activar `SESSION_RLS_ENABLED`** tras re-validar en staging.
- [~] **v42 — HU-156 · `tenant_id` tipado en escrituras (sin vistas sin tipar) · 🟡:** el `supabase gen types` completo rompía el build (el `types.ts` estaba **hand-augmented** con alias `Product`/`Order`/`ShippingAddress`/… que el código importa y que el generador no produce, + destapaba el bug latente de `order_items`). Decisión: conservar el `types.ts` actual y **añadir `tenant_id` a mano** en `customers`/`customer_addresses`/`orders` (Row/Insert/Update). Con eso se **eliminaron las vistas sin tipar**: `ensureCustomer` y el insert de dirección usan el cliente tipado; `createOrder` recibe `tenant_id?` y el checkout lo setea (pedido con el tenant correcto). Checkout resiliente: la resolución de tenant nunca bloquea el pago (fallback al default). Web **35·346** · database **18·171** · admin **13·169** verdes, type-check limpio en las 3 apps. **Deuda:** regeneración limpia de `types.ts` (separar Database generado de los alias a mano + arreglar `order_items`) queda como tarea aparte; borrar `packages/database/src/types.ts.bak`.
- [~] **v41 — HU-156 · Mutaciones `api/account/*` (perfil + direcciones) · 🟡:** `api/account/profile` (GET/PATCH) y `api/account/addresses` (GET/POST) cableados: `ensureCustomer` (garantiza fila → evita 404), cliente `authenticated` (RLS `customers_own`/`addresses_own`) cuando `SESSION_RLS_ENABLED` está activo (si no, service-role), y `tenant_id` explícito en el insert de dirección (calza `with check` + FK compuesta en cualquier tenant). `addresses/[id]` (PATCH/DELETE) se deja con ownership a nivel app (seguro) hasta regenerar `types.ts`. Test `profile.test.ts` actualizado a la nueva estructura. Web **35 suites · 346** verdes, type-check limpio. **Pendiente HU-156:** lecturas de checkout + `addresses/[id]` con authenticated + singletons no-catálogo + **regenerar `types.ts` con `tenant_id`** (para quitar las vistas sin tipar y setear tenant_id en todos los inserts tenant-scoped: `createOrder`, etc.).
- [~] **v40 — HU-156 · Enlace comprador↔customer↔order (bug de fondo) · 🟡:** validación de aislamiento en staging **OK** (A ve solo lo suyo, B solo lo suyo, lecturas cruzadas `[]`, insert en orders `42501`). Detectado y corregido: **nadie creaba `customers`** y el checkout no seteaba `orders.customer_id` (siempre null → con RLS el comprador no vería pedidos). `ensureCustomer()` (service-role, idempotente: stack_id→email→insert + **reclamo de pedidos de invitado** por email) en `@merkiai/database`; cableado en checkout (customer_id en las 3 `createOrder`) y en `/account` + `/account/orders` (provisioning + lectura por `customer_id`). Pruebas: `customers.test.ts` (3). Web **35 suites·346** + database **18 suites·171** verdes, type-check limpio. **Provider columns** de `orders` (skydropx_*/tucompra_*): genéricas y nullable — otras pasarelas/envíos quedan null, no rompe; deuda técnica de normalización anotada. **Pendiente HU-156:** `api/account/*` (profile/addresses) + lecturas de checkout + singletons no-catálogo. **No activar `SESSION_RLS_ENABLED`** hasta re-verificar que pedidos nuevos traen `customer_id` (flujo ya corregido) en staging.
- [~] **v39 — HU-156 · RLS de flujos con sesión (migración + lecturas de cuenta) · 🟡:** migración idempotente `e17/05_rls_session_flows.sql` (políticas `customers_own`/`addresses_own`/`cart_own`/`orders_own_read`, comprador `authenticated` ve solo lo suyo; `orders` SELECT-only). Gate seguro `getRequestUserDb(userId)` en `tenant-db.ts`: cliente `authenticated` solo con `SESSION_RLS_ENABLED='true'` + `SUPABASE_JWT_SECRET`; si no, `undefined`→server-role (cableado **inerte** hasta activar). **Lecturas de cuenta cableadas (gated):** `/account/orders`, `/account/orders/[id]` (+guardia por email), `/account` (dirección por defecto). Pruebas: `tenant-db.test.ts` (gate). Web **35 suites · 346 tests** verdes, type-check limpio. **Pendiente HU-156:** `api/account/*` (profile/addresses con `with check`), lecturas de checkout, y tenant-scope de singletons no-catálogo (`store_config`/`home_sections`/`pages`/`blog`). **Activación:** aplicar `e17/05` en staging + validar aislamiento A≠B, luego `SESSION_RLS_ENABLED=true`.
- [~] **v38 — HU-157 · Resolución host→tenant (núcleo) · 🟡:** `getRequestCatalogDb()` en `tenant-db.ts` con **degradación segura** (con `SUPABASE_JWT_SECRET` → cliente tenant-scoped rol `anon`+RLS por Host; sin secreto → `undefined` = server-role single-tenant). Home/catálogo/PDP (`(public)/page.tsx`, `shop/page.tsx`, `shop/[slug]/page.tsx`) a **render dinámico por request** con el tenant del Host. `resolveTenant` endurecido: caché por host con **TTL 60 s**, el fallback no se cachea. `defaultResolver()` usa `controlPlaneResolver` si hay `CONTROL_PLANE_URL`+`INTERNAL_API_SECRET`. **Pruebas:** `tenant-context.test.ts` (5 casos: sin env / parseo+secreto / no-ok / sin tenantId / fetch lanza). Web **34 suites · 344 tests** verdes, type-check limpio. Aislamiento real por RLS en catálogo (`categories`/`products`/`product_variants`). **Pendiente:** cola de URLs canónicas (SEO/emails/PaymentConfigForm/usuarios) y tenant-scope de singletons no-catálogo (`store_config`/`home_sections`/`pages`/`blog`) vía RLS (cola de HU-156). **Activación:** `SUPABASE_JWT_SECRET` + `e17/02_rls_catalog.sql` + `CONTROL_PLANE_URL`/`INTERNAL_API_SECRET` + DNS wildcard `*.merkiai.com`. Roadmap de olas actualizado con marcadores ✅/🟡/🔲.
- [x] **v37 — Console desplegado en producción (`merkiai.com`):** control plane live. Operador `platform:operate` otorgado vía `apps/console/scripts/grant-operator.mjs` (REST API de Stack Auth con `fetch`, sin SDK → evita el acople a Next en Node; `POST /api/v1/project-permissions/{userId}/{permissionId}` con cuerpo `{}`). Acceso `/login` → `/` verificado.
