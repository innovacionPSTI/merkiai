...
"[project]/apps/web/src/app/api/checkout/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
 ...
 */ async function saveAddressForUser(stackUserId, userEmail, shipping) {
    try {
        const supabase = (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$client$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createServerClient"])();
        // Buscar el customer
        let { data: customer } = await supabase.from('customers').select('id').eq('stack_id', stackUserId).maybeSingle();
        if (!customer) {
            const { data: byEmail } = await supabase.from('customers').select('id').eq('email', userEmail).maybeSingle();
            customer = byEmail;
        }
        if (!customer?.id) return;
        // Verificar si ya existe una dirección igual para no duplicar
        const { data: existing } = await supabase.from('customer_addresses').select('id').eq('customer_id', customer.id).eq('address', shipping.address).eq('city', shipping.city).maybeSingle();
        if (existing) return; // Ya guardada, no duplicar
        // Si no tiene dirección default, esta será la default
        const { data: hasDefault } = await supabase.from('customer_addresses').select('id').eq('customer_id', customer.id).eq('is_default', true).maybeSingle();
...
async function POST(req) {
    ...
    try {
        ...
        const { email, name, phone, address, items, subtotal, shipping_cost, total, // payment_method del cliente se ignora a propósito: la pasarela se deriva
        // del servidor (active_provider) por seguridad.
        discount, coupon_code, shipping_rate } = body;
        // Extract carrier info from the rate the user selected
        const skydropx_rate_id = shipping_rate?.id ?? null;
        const carrier_name = shipping_rate?.carrier_name ?? null;
        ...
        const stockRows = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$lib$2f$stock$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getStockForVariants"])(orderItems.map((i)=>i.variant_id).filter((v)=>!!v));
        const stockMap = new Map(stockRows.map((r)=>[
                r.variant_id,
                r
            ]));
        ...
        const paymentConfig = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$queries$2f$payment$2d$config$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getPaymentConfig"])();
        if (!paymentConfig) {
        ...
        const activeProvider = (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$providers$2f$payment$2f$index$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["getActiveProvider"])(paymentConfig);
        ...
        const autoSaveAddress = async ()=>{
            try {
                const sessionUser = await __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$stack$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["stackServerApp"].getUser();
        ...
        if (activeProvider === 'none') {
            const order = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$queries$2f$orders$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createOrder"])({
                customer_name: name,
        ...
        let gateway;
        try {
            gateway = (0, __TURBOPACK__imported__module__$5b$project$5d2f$packages$2f$database$2f$src$2f$providers$2f$payment$2f$index$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__$3c$locals$3e$__["getPaymentGateway"])(activeProvider, paymentConfig);
        } catch (err) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0mdv6xx._.js.map