/**
 * POST /api/admin/orders/[id]/reconcile-bold
 *
 ...
 */
import { reconcileBoldOrder } from '@merkiai/database'
...
export async function POST(
  ...
  if (order.payment_method !== 'bold') {
    return NextResponse.json({ error: 'Este pedido no es un pago Bold' }, { status: 400 })
  }

  try {
    const result = await reconcileBoldOrder(order.order_number)

    // Notificación al cliente si el pago quedó confirmado (fire-and-forget)
    if (result.status === 'approved') {
      void (async () => {
        try {
          ...
          console.error('[reconcile-bold] email:', e)
        }
      })()
    }
    ...
    console.error('[admin/reconcile-bold]', err)
    return NextResponse.json({ error: 'No se pudo consultar a Bold' }, { status: 502 })
  }
}
